...
- **Live Bug Tracker data source:** `collection://29d24bca-36d7-80ef-b574-000b739e37a8` (title "TeamBuildr OS 🪲 Tracker").
- **Canonical Accounts DB (created 2026-09-04):** **"Active Account Database"**, page `https://app.notion.com/p/3d124bca36d780f2941af6e143f5472e`, data source `collection://3d124bca-36d7-8069-a2fa-000b83deae3d`. Properties: `Account Name` (title), `Join Date` (date), `Status` (select: Active/Churned).

---

### Task 1: Canonical Accounts database in Notion — DONE (verify)

**Status:** ✅ Created manually 2026-09-04 as **"Active Account Database"** (`collection://3d124bca-36d7-8069-a2fa-000b83deae3d`) with `Account Name` (title), `Join Date` (date), `Status` (select: Active/Churned). Matches the schema needed by the mirror.

- [x] Database created with `Account Name` (title), `Join Date` (date), `Status` (select).
- [ ] **Optional:** add a `Source` text property ("Synced from Client List sheet — do not hand-edit") as a hand-edit guardrail. Cosmetic; skip if undesired.

---

### Task 2: Add the intake fields to the Bug Tracker — mostly DONE (verify)

**Deliverable:** the Bug Tracker captures accounts + the "Not actually a bug" severity, and the ticket template prompts for them.

- [x] **Relation added & verified 2026-09-04:** `Affected/Reported Accounts` (relation → Active Account Database, multi-value confirmed).
- [x] **Severity option added & verified:** `Priority Level` now = Not actually a bug / High / Moderate / Low.
- [ ] **Recommended follow-up:** make the relation **two-way (synced)** — toggle "Show on Active Account Database" — so each account page natively lists its related bug tickets (native per-account history for the weekly dev sync). Currently one-way.
...
**Deliverable:** the Active Account Database is populated with every current active account, so the intake dropdown works from day one.

- [ ] **Step 1:** `GET /api/accounts` (live Railway host) to get the full list.
- [ ] **Step 2:** For each account, create a page in the Active Account Database via Notion MCP `notion-create-pages` with `Account Name`, `Join Date`, `Status`, and the `Source` note. Batch in reasonable chunks.
...
**Deliverable:** a Claude scheduled task (via `mcp__scheduled-tasks__create_scheduled_task`) that runs daily and add/upserts new accounts into the Active Account Database.
...
> **Active Account Database Notion mirror.** Keep the Notion "Active Account Database" (data source `collection://3d124bca-36d7-8069-a2fa-000b83deae3d`) in sync with the account list in OS-Metric-Sync. Steps:
...
- [ ] **Step 2:** Verify: add a throwaway test account to the Client List sheet → `POST /api/refresh/accounts` → run the task → confirm the new account appears in the Active Account Database and then in the Bug Tracker's `Affected/Reported Accounts` dropdown. Remove the test account afterward.
