...
- **Canonical Accounts DB (created 2026-09-04):** **"OS Accounts Database"**, page `https://app.notion.com/p/3d124bca36d780f2941af6e143f5472e`, data source `collection://3d124bca-36d7-8069-a2fa-000b83deae3d`. Properties: `Account Name` (title), `Join Date` (date), `Status` (select: Active/Churned).
...
- [x] **Relation added & verified 2026-09-04:** `Affected/Reported Accounts` (relation → OS Accounts Database, multi-value confirmed).
- [x] **Severity option added & verified:** `Priority Level` now = Not actually a bug / High / Moderate / Low.
- [ ] **Recommended follow-up:** make the relation **two-way (synced)** — toggle "Show on OS Accounts Database" — so each account page natively lists its related bug tickets (native per-account history for the weekly dev sync). Currently one-way.
...
**Deliverable:** the OS Accounts Database is populated with every current active account, so the intake dropdown works from day one.

- [ ] **Step 1:** `GET /api/accounts` (live Railway host) to get the full list.
- [ ] **Step 2:** For each account, create a page in the OS Accounts Database via Notion MCP `notion-create-pages` with `Account Name`, `Join Date`, `Status`, and the `Source` note. Batch in reasonable chunks.
...
**Deliverable:** a Claude scheduled task (via `mcp__scheduled-tasks__create_scheduled_task`) that runs daily and add/upserts new accounts into the OS Accounts Database.
...
> **OS Accounts Database Notion mirror.** Keep the Notion "OS Accounts Database" (data source `collection://3d124bca-36d7-8069-a2fa-000b83deae3d`) in sync with the account list in OS-Metric-Sync. Steps:
...
- [ ] **Step 2:** Verify: add a throwaway test account to the Client List sheet → `POST /api/refresh/accounts` → run the task → confirm the new account appears in the OS Accounts Database and then in the Bug Tracker's `Affected/Reported Accounts` dropdown. Remove the test account afterward.
