...
- **Version:** v4 (bump on each meaningful seed/expansion; report cites `map v<n>`)
  ...
  - v4 (2026-08-17): added 4 reporting/data-coupling couplings from the at-risk-report run — attendance/sign-in both derive from a staff-set `booking.isSignedIn`; report date boundaries must use `studio.timeZone` (two date-range paths exist, one naive); "package" is a `PricingOption.type`-tagged Credit bundle and memberships also mint Credits; "cancellation" is overloaded (booking-status vs `contract.isCanceled` vs studio-initiated cascades).
